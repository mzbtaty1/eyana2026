...
return new class extends Migration
{
    ...
    {
        Schema::create('storage_statements', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->id();
            ...
            $table->foreignId('reversal_of_entry_id')->nullable()
                ->constrained('storage_statements')->nullOnDelete();
            $table->boolean('is_voided')->default(false);
            $table->unsignedBigInteger('created_by')->nullable();
            $table->timestamps();

            $table->index(['storage_id', 'transaction_date', 'id']);
        });
    }
    ...
    {
        Schema::dropIfExists('storage_statements');
    }
};
