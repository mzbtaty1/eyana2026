...
Route::middleware(['auth' , 'check_status'])->group(function(){
    ...
    // مسارات إدارة الفواتير المحسنة - جديدة
    Route::get('/invoices/management', [InvoiceController::class, 'management'])->name('invoices.management');
    Route::get('/get_invoices_management', [InvoiceController::class, 'getInvoices'])->name('invoices.management.data');
...
//   Route::get('/invoices/employee-log/print/{user_id}/{invoice_section?}/{date_from?}/{date_to?}/{airline_id?}/{travel_date?}/{invoice_beneficiaries?}/{vendor_id?}' , [InvoicesController::class , 'employee_log_print'])->name('site.employee_log_print');
...
//    Route::get('/apis/get_last_row/{sup_id}/{id}' , [APIsController::class ,'get_last_row'])->name('apis.get_last_row');
