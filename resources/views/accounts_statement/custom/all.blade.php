@extends('layouts.app')
@section('content')
@section('title' , "كشف حساب شركات مخصص")
...
<?php
$link = route('site.accounts_statement_custom_print' , [
...
$(document).ready(function() {
    let table = new DataTable('#InvoicesTable', {
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            var totalCredit = api.column(2, { search: 'applied' }).data()
                .reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);

            var totalDebit = api.column(3, { search: 'applied' }).data()
                .reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);

            var totalBalance = api.column(4, { search: 'applied' }).data()
                .reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);

            $(api.column(2).footer()).html(totalCredit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            $(api.column(3).footer()).html(totalDebit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            $(api.column(4).footer()).html(totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        }
    });
});
...
function printdiv(){
    var url = "{{$link}}"; 
    window.open(url, 'Print', 'width=900,height=600');
}

function copyText(TextToCopy, clientName){
    ...
    var text = rtl + "السادة/ " + clientName + " 💫🌸 برجاء العلم ان رصيدكم الحالي "
        + ltr + TextToCopy + end + " ج.م " + msg + end;
